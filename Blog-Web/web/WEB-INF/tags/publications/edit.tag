...
<form action="${pageContext.servletContext.contextPath}${action}" method="post">
    ...
    <div class="row gap-bottom">
        <textarea class="justify" name="content" placeholder="Content" style="width:1000px; height:450px"  required><c:if test = "${not empty publication}">${publication.content}</c:if></textarea>
    </div>
